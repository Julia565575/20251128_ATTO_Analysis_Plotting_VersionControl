...
for analyt_name, group in df.groupby(analyte_col):
    ...
    # Area at reference time
    area_ref = intercept

    # Define factor function string
    faktor_str = f"F(Δt) = {area_ref:.2f} / ({slope:.5f}·Δt + {intercept:.2f})"

    # Store in dataframe
    df.loc[df[analyte_col] == analyt_name, "factor_over_time"] = faktor_str
    df.loc[df[analyte_col] == analyt_name, "slope_Factor"] = slope
    df.loc[df[analyte_col] == analyt_name, "Intercept_Factor"] = intercept
    ...
    plt.figure(figsize=(6, 4))
    plt.plot(x_fit_datetime, faktor_values, color='green', 
             label=f"Korrekturfaktor\n{faktor_str}")
