...
for analyt_name, group in df.groupby(analyte_col):
    ...
    if r2 >= 0.7:
        ...
        # Alle möglichen Trennpunkte testen (außer die ersten/letzten 2 Punkte)
        for split_idx in range(2, len(x_seconds)-2):
            ...
            # Kombiniertes R² (alle Datenpunkte)
        ...
        x_split_val = x_seconds.iloc[best_split]

        # Fit-Linien berechnen
        x_fit_left = np.linspace(x_seconds.min(), x_split_val, 100)
        y_fit_left = slope1*x_fit_left + intercept1
        x_fit_right = np.linspace(x_split_val, x_seconds.max(), 100)
        y_fit_right = slope2*x_fit_right + intercept2
        ...
        plt.figure(figsize=(6, 4))
        plt.scatter(x_time, y_area, color='blue')
        plt.plot(x_fit_left_dt, y_fit_left, color='red',
                 label=f"LinFit1: y={slope1:.3e}·t+{intercept1:.2f}")
        plt.plot(x_fit_right_dt, y_fit_right, color='orange',
                 label=f"LinFit2: y={slope2:.3e}·t+{intercept2:.2f}")
        plt.axvline(split_time, color='gray', linestyle='--', label="Split point")
        plt.title(f"{analyt_name} – 2 Lineare Korrekturfunktionen")
        plt.xlabel("Messzeitpunkt (local time)")
        plt.ylabel("Area")
        plt.grid(True)
        plt.legend()
        ...
        dateiname = f"{datumsteil}_{analyt_name}_SIM_Korrekturfunktion_2linear.png"
        pfad = os.path.join(ordnername, dateiname)
        plt.savefig(pfad, bbox_inches='tight')
        plt.show()
        plt.close()
