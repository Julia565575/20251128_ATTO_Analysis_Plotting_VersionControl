...
#calibration_df = df[df['Unnamed: 4'].str.contains('dC')]
import pandas as pd
import matplotlib.pyplot as plt
import numpy as np
import re
...
# Define the full path to your CSV file
file_path = "C:/Users/julia.siebecker/Documents/ATTO_Data/2025040_alles Cal bisher/QuantReports/20250520_forPt/20250521_CalibrationData_AcqDateTime_in_local.csv"

# Step 1: Read the CSV
df = pd.read_csv(file_path)

# Filter calibration data
calibration_df = df[df['Unnamed: 4'].str.contains('directCal')]

# Filter specific measurements with Vol30 and SCAN mode
vol30_scan_df = df[df['Unnamed: 4'].str.contains('Vol30') & df['Unnamed: 4'].str.contains('SCAN')]

# Extract the relevant columns for plotting
calibration_df = calibration_df[['Unnamed: 4', 'Textbox2', 'Textbox2.1', 'Textbox3']]
vol30_scan_df = vol30_scan_df[['Unnamed: 4', 'Textbox2', 'Textbox2.1', 'Textbox3']]
...
for _, row in calibration_df.iterrows():
    plt.scatter(row['Textbox2.1'], row['Textbox3'], label=f"Calibration: {row['Unnamed: 4']}")

# Plot Vol30 SCAN measurements
for _, row in vol30_scan_df.iterrows():
    plt.scatter(row['Textbox2.1'], row['Textbox3'], label=f"Measurement: {row['Unnamed: 4']}")

plt.title("Calibration and Measurements Over Time")
plt.xlabel("Time")
plt.ylabel("Area")
plt.legend()
plt.grid(True)
...
# Save the plot
output_folder = "Calibration_Plots"
os.makedirs(output_folder, exist_ok=True)
plt.savefig(os.path.join(output_folder, "calibration_vs_measurements.png"))
plt.show()
