...
for analyt_name, group in df.groupby(analyte_col):
    ...
    # Dateiname & Pfad
    dateiname = f"{datumsteil}_{analyt_name}_nurCalibrierpunkte4.png"
...
for analyt_name, group in df.groupby(analyte_col):
    ...
    # Speichern
    dateiname = f"{datumsteil}_{analyt_name}_Korrekturfunktion4.png"
    pfad = os.path.join(ordnername, dateiname)
    plt.savefig(pfad, bbox_inches='tight')
    plt.show()
    plt.close()
...
for analyt_name, group in df.groupby(analyte_col):
    ...
    # Speichern
    dateiname = f"{datumsteil}_{analyt_name}_Proben4.png"
    pfad = os.path.join(ordnername, dateiname)
    plt.savefig(pfad, bbox_inches='tight')
    plt.show()
    plt.close()
...
# Debugging: Ausgabe aller Spaltennamen
print("Spalten im DataFrame:", df.columns.tolist())

# Define the function to extract slope
def extract_slope(equation):
    match = re.match(r'y\s*=\s*([+-]?\d*\.?\d*)x', equation)
    if match:
        slope = float(match.group(1))
        print(f"Extracted slope: {slope} from equation: {equation}")
        return slope
    print(f"No slope found in equation: {equation}")
    return None

# Apply the function to extract slope and create a new column
df['slope_korr'] = df['korr_CalFunction'].apply(extract_slope)

# Debugging: Check if the column is created and filled correctly
print(df[['korr_CalFunction', 'slope_korr']].head())

# Plot for each analyte
for analyt_name, group in df.groupby('analyte_col'):
    plt.figure(figsize=(10, 6))
    
    for typ, farbe in farben.items():
        # Überprüfen, ob 'datafile_name' existiert
        if 'datafile_name' not in group.columns:
            print(f"Missing 'datafile_name' in group for {analyt_name}")
            continue
        
        teilgruppe = group[group['datafile_name'].str.contains(typ, na=False)]
        
        # Check if 'slope_korr' has valid entries
        if teilgruppe['slope_korr'].isnull().all():
            print(f"No valid slopes for {analyt_name} and type {typ}")
            continue
        
        plt.scatter(
            teilgruppe['time_formated'],
            teilgruppe['slope_korr'],
            color=farbe,
        ...
        # Linear regression
        x_values = pd.to_datetime(teilgruppe['time_formated'], errors='coerce').map(pd.Timestamp.timestamp)
        y_values = teilgruppe['slope_korr']
        
        if not x_values.empty and not y_values.empty:
            # Fit line
            m, b = np.polyfit(x_values, y_values, 1)
            plt.plot(teilgruppe['time_formated'], m*x_values + b, color=farbe, linestyle='--')
            # Display slope and intercept
            plt.text(
                0.05, 0.95, f"Slope: {m:.5f}\nIntercept: {b:.2f}",
    ...
    plt.title(f"{analyt_name} – Slope über Zeit")
    plt.xlabel("Datum (time_formated)")
    plt.ylabel("Slope")
    plt.legend(title="Probentyp")
    plt.grid(True)
    plt.tight_layout()
    ...
    # Save the plot
    dateiname = f"{datumsteil}_{analyt_name}_Slope.png"
    pfad = os.path.join(ordnername, dateiname)
    plt.savefig(pfad, bbox_inches='tight')
    plt.show()
    plt.close()