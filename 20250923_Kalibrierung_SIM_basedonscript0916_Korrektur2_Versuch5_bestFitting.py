...
ordnername = f"{datumsteil}_Kalibrierung_SIM_Korrektur2_Versuch_bestFit"              ###########################################################################
...
#%%
# =============================================================================
# Korrekturfunktion aus den dC Proben plotten und errechnen. BEST FIT
# =============================================================================
# =============================================================================
# Beste Korrekturfunktion (linear, quadratisch, exponentiell) automatisch wählen
# =============================================================================
import os
import numpy as np
import pandas as pd
...
for analyt_name, group in df.groupby(analyte_col):
    ...
    models = {
        "Linear": (linear, [1, 1]),
        "Quadratisch": (quadratic, [1, 1, 1]),
        "Exponentiell": (exponential, [1, 0.001, 1])
    }
    
    #TO TEST:
        #Für Drift-Korrekturen (wie bei dir) reichen meistens linear, quadratisch, exponentiell.
        # Wenn du Sättigungseffekte erwartest → logistisch oder logarithmisch.
        # Wenn du periodische Schwankungen hast → Sinus.
        # Peaks in Chromatographie → Gauß.
    ...
    for name, (func, p0) in models.items():
        try:
            params, _ = curve_fit(func, x_seconds, y_area, p0=p0, maxfev=5000)
            residuals = y_area - func(x_seconds, *params)
    ...
    plt.plot(x_fit_datetime, y_fit, color='red',
             label=f"{best_name}, R² = {best_r2:.3f}")
    plt.plot([], [], ' ', label=rf"$y_{{t_0}}$ = {referenz_zeitpunkt_str}")
    plt.title(f"{analyt_name} – Bester Fit")
    ...
    # Speichern
    dateiname = f"{datumsteil}_{analyt_name}_SIM_Korrekturfunktion_bestFit.png"
    pfad = os.path.join(ordnername, dateiname)
    plt.savefig(pfad, bbox_inches='tight')
    plt.show()
    plt.close()
    ...
    df.loc[df[analyte_col] == analyt_name, "Korrekturfunktion"] = best_name
