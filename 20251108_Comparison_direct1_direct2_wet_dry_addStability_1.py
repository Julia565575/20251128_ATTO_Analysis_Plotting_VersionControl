...
#%%    
# =============================================================================
# Plot stability measurements: combined view per analyte (robust to extra tokens)
# =============================================================================

# # pattern to identify stability-type files (non-capturing group -> no warning)
# stability_type_pattern = r'_(?:directStability|wetStability|dryStability)_'
...
for analyte_name in selected_analytes:
    ...
    for label, df_current in dfs.items():
        ...
        # --- Filter stability measurements by the stability-type pattern ---
        stability_group = group[group[datafile_col].str.contains(r'dryStability_', case=False, na=False, regex=True)]


        # Debug: show which files matched (helpful if no plot appears)
        if not stability_group.empty:
            print(f"[{analyte_name}] {label} matched {len(stability_group)} files:")
            print(" ", stability_group[datafile_col].tolist())

        if stability_group.empty:
            continue

        # --- Extract the measurement number robustly ---
        # Strategy: find all digit groups in the filename and use the last one.
        # This handles names like ..._SIM_Vol5_1 and ..._SIM_SA150_CG5_20
        stability_group['measurement_num'] = (
            stability_group[datafile_col]
            .str.findall(r'(\d+)')                        # list of digit-groups per filename
            .apply(lambda lst: int(lst[-1]) if lst else np.nan)  # take last group, convert to int
        )

        # Convert area to numeric and drop rows with missing data
        stability_group['area_num'] = pd.to_numeric(stability_group[y_col], errors='coerce')
        stability_group = stability_group.dropna(subset=['measurement_num', 'area_num'])

        if stability_group.empty:
            # Nothing numeric left after cleaning
            continue

        # Sort by measurement number so lines are in order
        stability_group = stability_group.sort_values('measurement_num')

        x = stability_group['measurement_num'].values
        y = stability_group['area_num'].values

        # Plot with markers
        plt.plot(x, y, marker='o', linestyle='-', label=f"{label}")
        any_plotted = True

    if not any_plotted:
        # no stability data for this analyte across all dfs
        plt.close()
        continue

    plt.title(f"{analyte_name} — Stability Over Measurements")
    ...
    # Save figure
    filename = f"{datumsteil}_{analyte_name}_SIM_stability.png"
    path = os.path.join(ordnername, filename)
