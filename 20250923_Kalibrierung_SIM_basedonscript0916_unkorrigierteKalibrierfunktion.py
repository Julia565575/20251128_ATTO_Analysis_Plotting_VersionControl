...
ordnername = f"{datumsteil}_Kalibrierung_SIM_Korrektur2_Versuch1"              ###########################################################################
...
for analyt_name, group in df.groupby(analyte_col):
    ...
    # Save
    dateiname = f"{datumsteil}_{analyt_name}_SIM_Corr_vs_Uncorr.png"
    pfad = os.path.join(ordnername, dateiname)
    plt.savefig(pfad, bbox_inches='tight')
    plt.show()
    plt.close()
...
for analyt_name, group in df.groupby(analyte_col):
    ...
    referenz_zeitpunkt_str = "2025-05-13 16:14"         #local time; because times are plotted in local
...
#%%
# =============================================================================
# Correction factor function F(t) for every analyte
# =============================================================================

# Create new columns to store correction factor functions
df["Korrektur_Faktor_Funktion"] = np.nan

for analyt_name, group in df.groupby(analyte_col):
    dC_Korrektur = group[group[datafile_col].str.contains('_dC_', na=False)]

    # Zeit und Peakfläche wie zuvor
    x_time = pd.to_datetime(dC_Korrektur['time_formated_local'], errors='coerce')
    y_area = pd.to_numeric(dC_Korrektur[y_col], errors='coerce')
    ...
    # Area at reference time = intercept (since at t0 → x=0)
    area_ref = intercept

    # Define correction factor function
    def faktor_funktion(t):
        """Return correction factor for given datetime t (local time)."""
        if pd.isna(t):
            return np.nan
        seconds = (pd.to_datetime(t) - t0).total_seconds()
        area_t = slope * seconds + intercept
        if area_t == 0:
            return np.nan
        return area_ref / area_t

    # Store function as lambda string (optional, for traceability in df)
    df.loc[df[analyte_col] == analyt_name, "Korrektur_Faktor_Funktion"] = \
        f"F(t) = {area_ref:.2f} / ({slope:.5f}·(t-t0) + {intercept:.0f})"

    # === Plot correction factor over time ===
    x_fit = np.linspace(x_seconds.min(), x_seconds.max(), 200)
    faktor_values = [area_ref / (slope * xx + intercept) for xx in x_fit]
    x_fit_datetime = t0 + pd.to_timedelta(x_fit, unit='s')

    plt.figure(figsize=(6, 4))
    plt.plot(x_fit_datetime, faktor_values, color='green', label="Korrekturfaktor")
    ...
    dateiname = f"{datumsteil}_{analyt_name}_SIM_Korrekturfaktor.png"
    pfad = os.path.join(ordnername, dateiname)
    plt.savefig(pfad, bbox_inches='tight')
    plt.show()
    plt.close()
...
#%%
# =============================================================================
# Apply correction factors to all rows -> area_corrected
# =============================================================================

df["Korrektur_Faktor"] = np.nan
df["area_corrected"] = np.nan

for analyt_name, group in df.groupby(analyte_col):
    # Get slope and intercept for this analyte from the fitted correction function
    fkt_str = group["Korrekturfunktion"].dropna().unique()
    if len(fkt_str) == 0:
        continue

    # Parse slope and intercept back from the string "y = slope·t + intercept"
    try:
        slope_str, intercept_str = fkt_str[0].split("·t +")
        slope = float(slope_str.split("=")[1].strip())
        intercept = float(intercept_str.strip())
    except Exception as e:
        print(f"Could not parse Korrekturfunktion for {analyt_name}: {fkt_str}")
        continue

    # Area at reference time (t0) = intercept
    area_ref = intercept

    for idx, row in group.iterrows():
        t = row["time_formated_local"]
        area = row["area"]

        if pd.isna(t) or pd.isna(area):
            continue

        # Time difference in seconds relative to t0
        delta_t = (t - t0).total_seconds()

        # Predicted area at this time
        area_t = slope * delta_t + intercept
        if area_t == 0:
            continue

        # Correction factor and corrected area
        faktor = area_ref / area_t
        df.at[idx, "Korrektur_Faktor"] = faktor
