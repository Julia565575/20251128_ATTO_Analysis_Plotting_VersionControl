...
ordnername = f"{datumsteil}_Kalibrierung_SIM_Korrektur2_Versuch_quadratischOder2Lineare"              ###########################################################################
...
#%%
# =============================================================================
# Korrekturfunktion aus den dC Proben plotten und errechnen. QUADRATISCH ODER 2LINEARE
# =============================================================================
# =============================================================================
# Quadratische Korrekturfunktion oder 2 lineare Fits (wenn R² < 0.7)
# =============================================================================
import os
import numpy as np
import pandas as pd
...
df["Fit_Typ"] = np.nan
df["Korrekturfunktion"] = np.nan
df["Korr_Fkt_R^2"] = np.nan

df["a_factor"] = np.nan
df["b_factor"] = np.nan
df["c_factor"] = np.nan

df["Lin1_Funktion"] = np.nan
df["slope1_factor"] = np.nan
df["intercept1_factor"] = np.nan
df["Lin2_Funktion"] = np.nan
df["slope2_factor"] = np.nan
df["intercept2_factor"] = np.nan

df["date_split"] = pd.NaT  # neue Spalte für den Split-Zeitpunkt
...
for analyt_name, group in df.groupby(analyte_col):
    ...
    # =============================
    # 1) Quadratisch fitten
    # =============================
    params, _ = curve_fit(quadratic, x_seconds, y_area)
    a, b, c = params
    y_pred_quad = quadratic(x_seconds, a, b, c)
    r2 = calc_r2(y_area, y_pred_quad)

    if r2 >= 0.7:
        # -------------------
        # Quadratischer Fit ok
        # -------------------
        formel_str = f"y = {a:.3e}·t² + {b:.3e}·t + {c:.2f}"
        df.loc[df[analyte_col] == analyt_name, "Fit_Typ"] = "Quadratisch"
        df.loc[df[analyte_col] == analyt_name, "Korrekturfunktion"] = formel_str
        df.loc[df[analyte_col] == analyt_name, "Korr_Fkt_R^2"] = f"{r2:.3f}"
        df.loc[df[analyte_col] == analyt_name, "a_factor"] = a
        df.loc[df[analyte_col] == analyt_name, "b_factor"] = b
        df.loc[df[analyte_col] == analyt_name, "c_factor"] = c

    else:
        # =============================
        # 2) Quadratisch schlecht → 2 lineare Fits
        # =============================
        best_split = None
        best_r2 = -np.inf
        best_models = None

        for split_idx in range(2, len(x_seconds)-2):
            x_left, y_left = x_seconds[:split_idx], y_area[:split_idx]
            x_right, y_right = x_seconds[split_idx:], y_area[split_idx:]

            slope1, intercept1, _, _, _ = linregress(x_left, y_left)
            slope2, intercept2, _, _, _ = linregress(x_right, y_right)

            y_pred_left = slope1*x_left + intercept1
            y_pred_right = slope2*x_right + intercept2

            y_pred_all = np.concatenate([y_pred_left, y_pred_right])
            y_true_all = np.concatenate([y_left, y_right])
            r2_split = calc_r2(y_true_all, y_pred_all)

            if r2_split > best_r2:
                best_r2 = r2_split
                best_split = split_idx
                best_models = (slope1, intercept1, slope2, intercept2)

        slope1, intercept1, slope2, intercept2 = best_models
        split_time = x_time.iloc[best_split]

        lin1_str = f"y = {slope1:.3e}·t + {intercept1:.2f}"
        lin2_str = f"y = {slope2:.3e}·t + {intercept2:.2f}"
        df.loc[df[analyte_col] == analyt_name, "Fit_Typ"] = "2-linear"
        df.loc[df[analyte_col] == analyt_name, "Korrekturfunktion"] = f"{lin1_str} | {lin2_str}"
        df.loc[df[analyte_col] == analyt_name, "Korr_Fkt_R^2"] = f"{best_r2:.3f}"
        df.loc[df[analyte_col] == analyt_name, "Lin1_Funktion"] = lin1_str
        df.loc[df[analyte_col] == analyt_name, "slope1_factor"] = slope1
        df.loc[df[analyte_col] == analyt_name, "intercept1_factor"] = intercept1
        df.loc[df[analyte_col] == analyt_name, "Lin2_Funktion"] = lin2_str
        df.loc[df[analyte_col] == analyt_name, "slope2_factor"] = slope2
        df.loc[df[analyte_col] == analyt_name, "intercept2_factor"] = intercept2
        df.loc[df[analyte_col] == analyt_name, "date_split"] = split_time
...
#%%
# =============================================================================
# Correction factor function F(t) – je nach Fit-Typ (Quadratisch oder 2-linear)
# =============================================================================
from scipy.optimize import curve_fit
...
# Neue Spalten vorbereiten
df["factor_function_str"] = np.nan
df["a_Factor"] = np.nan
df["b_Factor"] = np.nan
df["c_Factor"] = np.nan

for analyt_name, group in df.groupby(analyte_col):
    fit_typ = group["Fit_Typ"].iloc[0]
    dC_Korrektur = group[group[datafile_col].str.contains('_dC_', na=False)]
    ...
    t0 = pd.to_datetime("2025-05-13 16:14")
    x_seconds = (x_time - t0).dt.total_seconds()

    # =====================================================
    # Quadratischer Fit
    # =====================================================
    if fit_typ == "Quadratisch":
        ...
        # DataFrame updaten
        df.loc[df[analyte_col] == analyt_name, "factor_function_str"] = faktor_str
        ...
        plt.title(f"{analyt_name} – Korrekturfaktor über Zeit (quadratisch)")
    ...
    # 2-lineare Fits
    # =====================================================
    elif fit_typ == "2-linear":
        slope1 = group["slope1_factor"].iloc[0]
        intercept1 = group["intercept1_factor"].iloc[0]
        slope2 = group["slope2_factor"].iloc[0]
        intercept2 = group["intercept2_factor"].iloc[0]
        split_time = group["date_split"].iloc[0]
        split_seconds = (split_time - t0).total_seconds()
    
        # Referenzwert (bei Δt=0 auf erster Geraden)
        area_ref = slope1*0 + intercept1
    
        # Faktoren berechnen
        faktor_values = []
        for xx, xtime in zip(x_seconds, x_time):
            if xtime <= split_time:
                area_t = slope1*xx + intercept1
            else:
                area_t = slope2*xx + intercept2
            faktor_values.append(area_ref / area_t if area_t != 0 else np.nan)
    
        # R² für beide Fits berechnen
        y_left = slope1*x_seconds[x_time <= split_time] + intercept1
        y_right = slope2*x_seconds[x_time > split_time] + intercept2
        r2_left = 1 - np.sum((y_area[x_time <= split_time] - y_left)**2) / np.sum((y_area[x_time <= split_time] - np.mean(y_area[x_time <= split_time]))**2)
        r2_right = 1 - np.sum((y_area[x_time > split_time] - y_right)**2) / np.sum((y_area[x_time > split_time] - np.mean(y_area[x_time > split_time]))**2)
    
        # Legendenstrings mit Formel + R²
        lin1_str = f"LinFaktor 1: y={slope1:.3e}·t + {intercept1:.2f}, R²={r2_left:.3f}"
        lin2_str = f"LinFaktor 2: y={slope2:.3e}·t + {intercept2:.2f}, R²={r2_right:.3f}"
    
        # DataFrame updaten
        df.loc[df[analyte_col] == analyt_name, "factor_function_str"] = "piecewise linear factor"
    
        # Plot vorbereiten
        x_fit_left = np.linspace(x_seconds.min(), split_seconds, 100)
        x_fit_right = np.linspace(split_seconds, x_seconds.max(), 100)
    
        y_fit_left = [area_ref / (slope1*xx + intercept1) for xx in x_fit_left]
        y_fit_right = [area_ref / (slope2*xx + intercept2) for xx in x_fit_right]
    
        x_fit_left_dt = t0 + pd.to_timedelta(x_fit_left, unit='s')
        x_fit_right_dt = t0 + pd.to_timedelta(x_fit_right, unit='s')
    
        plt.figure(figsize=(6, 4))
        plt.scatter(x_time, faktor_values, color='green', alpha=0.7, label="Faktoren (berechnet)")
        plt.plot(x_fit_left_dt, y_fit_left, color='red', label=lin1_str)
        plt.plot(x_fit_right_dt, y_fit_right, color='orange', label=lin2_str)
        plt.axhline(1, color='gray', linestyle='--', label="Reference = 1.0")
        plt.axvline(split_time, color='gray', linestyle='--', label="Split point")
        plt.title(f"{analyt_name} – Korrekturfaktor über Zeit (2-linear)")
        plt.xlabel("Messzeitpunkt (local time)")
        plt.ylabel("Correction factor F(t)")
        plt.grid(True)
        plt.legend()
        ...
        dateiname = f"{datumsteil}_{analyt_name}_SIM_Korrekturfaktor_2linear.png"
        pfad = os.path.join(ordnername, dateiname)
        plt.savefig(pfad, bbox_inches='tight')
        plt.show()
        plt.close()
...
# =============================================================================
# =============================================================================
# Berechnung des Korrektur-Faktors je Zeile
# =============================================================================
...
for analyt_name, group in df.groupby(analyte_col):
    fit_typ = group["Fit_Typ"].iloc[0]
    date_split = group["date_split"].iloc[0] if "date_split" in group.columns else pd.NaT

    # Parameter für quadratische bzw. lineare Fits
    aF = group["a_Factor"].iloc[0]
    bF = group["b_Factor"].iloc[0]
    cF = group["c_Factor"].iloc[0]
    ...
    for idx, row in group.iterrows():
        ...
        if fit_typ == "Quadratisch":
            if pd.notna(aF) and pd.notna(bF) and pd.notna(cF):
                faktor = aF * delta_t**2 + bF * delta_t + cF
                df.at[idx, 'Korrektur_Factor'] = faktor

        elif fit_typ == "2-linear":
            if pd.isna(date_split) or pd.isna(slope1) or pd.isna(intercept1) or pd.isna(slope2) or pd.isna(intercept2):
                continue

            # Welcher lineare Fit?
            if t < date_split:
                faktor = slope1 * delta_t + intercept1
            else:  # t >= date_split → Fit 2
                faktor = slope2 * delta_t + intercept2
