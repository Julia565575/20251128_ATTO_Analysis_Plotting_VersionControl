...
df["slope_Factor"] = np.nan
df["Intercept_Factor"] = np.nan
...
for analyt_name, group in df.groupby(analyte_col):
    ...
    # Erste Regression: Area drift
    slope, intercept, r_value, p_value, std_err = linregress(x_seconds, y_area)
    area_ref = intercept

    # === Faktoren berechnen ===
    faktor_values = [area_ref / (slope * xx + intercept) for xx in x_seconds]

    # === Neue Regression: Faktor über Zeit ===
    slope_factor, intercept_factor, r_val_f, p_val_f, std_err_f = linregress(x_seconds, faktor_values)

    faktor_str = f"F(Δt) = {slope_factor:.5e}·Δt + {intercept_factor:.5f}"

    # Store results
    df.loc[df[analyte_col] == analyt_name, "factor_over_time"] = faktor_str
    df.loc[df[analyte_col] == analyt_name, "slope_Factor"] = slope_factor
    df.loc[df[analyte_col] == analyt_name, "Intercept_Factor"] = intercept_factor

    # === Plot ===
    x_fit = np.linspace(x_seconds.min(), x_seconds.max(), 200)
    y_fit_factor = slope_factor * x_fit + intercept_factor
    x_fit_datetime = t0 + pd.to_timedelta(x_fit, unit='s')

#green points for when the CG measurements happenend (but based on the fit)
    plt.figure(figsize=(6, 4))
    ...
    dateiname = f"{datumsteil}_{analyt_name}_SIM_Korrekturfaktor_Korrektur2.png"
    pfad = os.path.join(ordnername, dateiname)
    plt.savefig(pfad, bbox_inches='tight')
    plt.show()
    plt.close()
...
#%%
# =============================================================================
# Calculate correction factor for each row
# =============================================================================

import pandas as pd
import numpy as np

# Make sure 'time_formated_local' is datetime
df['time_formated_local'] = pd.to_datetime(df['time_formated_local'], errors='coerce')

# Ensure slope_Factor and Intercept_Factor exist
df['slope_Factor'] = pd.to_numeric(df['slope_Factor'], errors='coerce')
df['Intercept_Factor'] = pd.to_numeric(df['Intercept_Factor'], errors='coerce')

# Create new column for factor
df['Korrektur_Factor'] = np.nan

# Loop over each row
for idx, row in df.iterrows():
    slope = row.get('slope_Factor')
    intercept = row.get('Intercept_Factor')
    t = row.get('time_formated_local')

    # Skip rows with missing data
    if pd.isna(slope) or pd.isna(intercept) or pd.isna(t):
        continue
    ...
    # Factor = m * Δt + b
    faktor = slope * delta_t + intercept

    # Store factor in dataframe
    df.at[idx, 'Korrektur_Factor'] = faktor

# Optional: check first few rows
print(df[['time_formated_local', 'slope_Factor', 'Intercept_Factor', 'Korrektur_Factor']].head())
