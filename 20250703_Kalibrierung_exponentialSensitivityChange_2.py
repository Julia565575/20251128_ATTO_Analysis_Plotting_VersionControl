...
file_path = "C:/Users/julia.siebecker/Documents/ATTO_Data/Workflow_full_dataset_20250430/QuantReports/Trial_20250701_tryAnalyseForExport/20250701_CalibrationData_AcqDateTime_local_test_ful_3analytes.csv"
...
for analyt_name, group in df.groupby(analyte_col):
    ...
    # Dateiname & Pfad
    dateiname = f"{datumsteil}_{analyt_name}_nurCalibrierpunkte_exp.png"
...
# For each analyte, plot the time series and perform exponential fitting
for analyt_name, group in df.groupby(analyte_col):
    ...
    # Convert time to seconds relative to the reference time
    ...
    try:
        popt, pcov = curve_fit(exponential_func, x_seconds, y_area, p0=(1, -1e-6, 1))
        a, b, c = popt
        ...
        plt.plot(x_fit_datetime, y_fit, color='red',
                 label=f"y = {a:.5f} * exp({b:.5e} * t) + {c:.5f}")
        plt.title(f"{analyt_name} – Exponential Fit")
        plt.xlabel("Measurement Time (notUTC)")
        plt.ylabel("Area")
        plt.grid(True)
        plt.legend()
        plt.tight_layout()
        ...
        # Save the plot
        dateiname = f"{datumsteil}_{analyt_name}_ExponentialFit_exp.png"
        pfad = os.path.join(ordnername, dateiname)
        plt.savefig(pfad, bbox_inches='tight')
        plt.show()
        plt.close()

        # Save the fit equation in the DataFrame
        formel_str = f"y = {a:.5f} * exp({b:.5e} * t) + {c:.5f}"
        df.loc[df[analyte_col] == analyt_name, "Korrekturfunktion"] = formel_str

    except RuntimeError:
        print(f"Exponential fit failed for {analyt_name}")
...
#     # Speichern
#     dateiname = f"{datumsteil}_{analyt_name}_Korrekturfunktion_exp.png"
#     pfad = os.path.join(ordnername, dateiname)
#     plt.savefig(pfad, bbox_inches='tight')
#     plt.show()
#     plt.close()
...
# Function to calculate the correction value per row using exponential function
def berechne_korr_zeitwert(row):
    ...
    try:
        # Extract parameters from the exponential function string
        match = re.match(r"y\s*=\s*([-+]?[0-9]*\.?[0-9]+)\s*\*\s*exp\(\s*([-+]?[0-9]*\.?[0-9]+)e?([-+]?[0-9]+)?\s*\*\s*t\)\s*\+\s*([-+]?[0-9]*\.?[0-9]+)", fkt)
        if match:
            a = float(match.group(1))
            b = float(match.group(2))
            if match.group(3):  # Handle scientific notation
                b *= 10**int(match.group(3))
            c = float(match.group(4))
            
            # Calculate time difference in seconds
            delta_t = (zeitpunkt - t0).total_seconds()
            
            # Calculate the area using the exponential function
            return a * np.exp(b * delta_t) + c
        else:
            print(f"Regex did not match for function: {fkt}")
...
for analyt_name, group in df.groupby(analyte_col):
    ...
    # Speichern
    dateiname = f"{datumsteil}_{analyt_name}_Proben_exp.png"
    pfad = os.path.join(ordnername, dateiname)
    plt.savefig(pfad, bbox_inches='tight')
    plt.show()
    plt.close()
...
#         # Extract time component for daily pattern
#         times_of_day = teilgruppe['time_formated'].dt.time
...
#     # X-axis formatting for time of day
#     ax = plt.gca()
...
#     # Save
#     dateiname = f"{datumsteil}_{analyt_name}_DailyPattern.png"
#     pfad = os.path.join(ordnername, dateiname)
#     plt.savefig(pfad, bbox_inches='tight')
#     plt.show()
#     plt.close()
...
for analyt_name, group in df.groupby(analyte_col):
    ...
    # Save
    dateiname = f"{datumsteil}_{analyt_name}_DailyPattern_exp.png"
    pfad = os.path.join(ordnername, dateiname)
    plt.savefig(pfad, bbox_inches='tight')
    plt.show()
    plt.close()
