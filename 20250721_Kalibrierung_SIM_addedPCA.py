...
# --- STEP 1: Filter out standard measurements ---
mask = ~df['datafile_name'].str.contains('_dC_|_directCal_', regex=True)
df_filtered = df[mask].copy()

# --- STEP 2: Pivot data so rows=samples, columns=analytes ---
pivot_df = df_filtered.pivot(index='datafile_name', columns='analyte_col', values='Conc')

# --- STEP 3: Impute missing values with mean ---
imputer = SimpleImputer(strategy='mean')
pivot_df_imputed = pd.DataFrame(imputer.fit_transform(pivot_df),
                               index=pivot_df.index,
                               columns=pivot_df.columns)

# --- STEP 4: PCA based on Spearman correlation ---

# Compute Spearman correlation matrix between analytes
...
# --- STEP 5: PCA based on Z-score normalized analyte concentrations ---

# Standardize analytes (mean=0, std=1)
...
zscore_scores = pca_zscore.fit_transform(z_scaled_df.T)  # transpose so analytes are variables
...
# Plot PCA from Spearman correlation
...
axs[0].set_title("PCA Based on Spearman Correlation")
...
# Plot PCA from Z-score normalized data
...
axs[1].set_title("PCA Based on Z-score Normalization")
