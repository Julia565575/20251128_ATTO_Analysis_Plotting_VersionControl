...
#%%
# =============================================================================
# Plot stability: combined view per analyte
# =============================================================================
# =============================================================================
# Plot stability measurements: combined view per analyte (sequential x-axis)
# =============================================================================
...
#     for label, df_current in dfs.items():
#         # Filter current compound
#         group = df_current[df_current[analyte_col] == analyte_name]

#         # Filter stability measurements (adjust this if you also have wet/direct2 later)
#         # stability_group = group[group[datafile_col].str.contains(
#         #     r'_(?:directStability|dryStability)_', case=False, na=False, regex=True
#         # )].copy()
#         stability_group = group[group[datafile_col].str.contains(r'Stability', case=False, na=False)].copy()
#         #stability_group = group[group[datafile_col].str.contains(r'_(directStability_|dryStability_)', case=False, na=False, regex=True)]
...
#         stability_group[y_col] = pd.to_numeric(stability_group[y_col], errors='coerce')
#         stability_group = stability_group.dropna(subset=[y_col])

#         if stability_group.empty:
#             continue

#         # Sort by filename for consistent ordering
#         stability_group = stability_group.sort_values(by=datafile_col).reset_index(drop=True)

#         # Sequential x-axis: 1, 2, 3, ... N
#         x = np.arange(1, len(stability_group) + 1)
#         y = stability_group[y_col].values
...
#     plt.title(f"{analyte_name} — Stability Measurements")
...
for analyte_name in selected_analytes:
    ...
    for label, df_current in dfs.items():
        ...
        x = x = np.arange(1, len(stability_group) + 1)
        y = pd.to_numeric(cal_group[y_col], errors='coerce')
        valid =  y.notna()
        y = y[valid].values
