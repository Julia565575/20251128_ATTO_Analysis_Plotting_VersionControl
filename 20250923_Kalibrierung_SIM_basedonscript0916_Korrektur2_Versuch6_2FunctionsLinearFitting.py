...
for analyt_name, group in df.groupby(analyte_col):
    ...
    if r2 >= 0.7:
        ...
        x_split_val = x_seconds.iloc[best_split]   # <<< Fix 1

        # Fit-Linien berechnen
        x_fit_left = np.linspace(x_seconds.min(), x_split_val, 100)  # <<< Fix 2
        y_fit_left = slope1*x_fit_left + intercept1
        x_fit_right = np.linspace(x_split_val, x_seconds.max(), 100)  # <<< Fix 3
        ...
        # In DataFrame speichern
        df.loc[df[analyte_col] == analyt_name, "Korrekturfunktion"] = (
            f"Piecewise linear (Split @ {split_time})"
        )
