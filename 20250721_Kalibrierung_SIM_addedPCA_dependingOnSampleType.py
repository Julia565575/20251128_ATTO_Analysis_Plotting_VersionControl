...
ordnername = f"{datumsteil}_Kalibrierung_SIM_wPCA"
...
for analyt_name, group in df.groupby(analyte_col):
    ...
    x_time = pd.to_datetime(dC_Korrektur['time_formated'], errors='coerce')
...
for analyt_name, group in df.groupby(analyte_col):
    ...
    plt.title(f"{analyt_name} – Konzentration über Zeit")
    plt.xlabel("Datum (time_formated)")
    plt.ylabel("Berechnete Konzentration [ppb]")
    plt.legend(title="Probentyp")  # Hier wird die Legende angezeigt
    plt.grid(True)
    plt.tight_layout()
    ...
    # =========================
    x_time = pd.to_datetime(group['time_formated'], errors='coerce')
...
for analyt_name, group in df.groupby(analyte_col):
    ...
    for typ, farbe in farben.items():
        ...
        hours_since_midnight = teilgruppe['time_formated'].dt.hour + teilgruppe['time_formated'].dt.minute / 60 + teilgruppe['time_formated'].dt.second / 3600
    ...
    plt.title(f"{analyt_name} – Daily Concentration Pattern")
    plt.xlabel("Hours Since Midnight")
    plt.ylabel("Calculated Concentration [ppb]")
...
# # STEP 1: Pivot the data (if not already done)
# pivot_df = df.pivot(index='datafile_name', columns='analyte_col', values='Conc')

# # STEP 2: Drop missing values (or fill with mean if preferred)
# pivot_df_clean = pivot_df.dropna()

# # STEP 3: Calculate pairwise Pearson correlations between analytes
# correlation_matrix = pivot_df_clean.corr(method='pearson')
...
#             vmin=0, vmax=1, 
#             linewidths=0.5, 
#             cbar_kws={"label": "Correlation (Covariation)"})

# plt.title("Analyte Covariation Heatmap (Pearson Correlation)", fontsize=14)
...
# pivot_df_clean = pivot_df.dropna()  # or: .fillna(pivot_df.mean())

# # STEP 3: Compute correlation matrix
# correlation_matrix = pivot_df_clean.corr(method='pearson')

# # STEP 4: Plot heatmap with custom sizing and font
# num_analytes = correlation_matrix.shape[0]
# cell_size = 0.5  # Adjust this to make cells larger (0.5–1 is typical)
# fig_width = num_analytes * cell_size
# fig_height = num_analytes * cell_size
...
#             vmin=0, vmax=1,
#             linewidths=0.4,
#             annot_kws={"size": 6},  # Smaller text for numbers
#             cbar_kws={"label": "Correlation (Covariation)"})
...
mask_standards = ~df['datafile_name'].str.contains('_dC_|_directCal_', regex=True)
df_filtered = df[mask_standards].copy()

# Clean analyte names: lowercase and strip spaces to avoid duplicates from typos
df_filtered['analyte_col'] = df_filtered['analyte_col'].str.strip().str.lower()

time_groups = ['_0m_', '_80m_', '_320m_']

for time_tag in time_groups:
    print(f"\nProcessing samples with '{time_tag}'")

    mask_time = df_filtered['datafile_name'].str.contains(time_tag)
    df_time = df_filtered[mask_time].copy()

    pivot_df = df_time.pivot(index='datafile_name', columns='analyte_col', values='Conc')

    print("Before removing duplicates:", pivot_df.shape)
    print("Duplicate columns:", pivot_df.columns[pivot_df.columns.duplicated()].tolist())

    pivot_df = pivot_df.loc[:, ~pivot_df.columns.duplicated()]

    print("After removing duplicates:", pivot_df.shape)

    if pivot_df.empty:
        print(f"No data for {time_tag}, skipping.")
        continue

    # Impute missing values
    imputer = SimpleImputer(strategy='mean')
    imputed_array = imputer.fit_transform(pivot_df)
    print("Imputed array shape:", imputed_array.shape)

    pivot_df_imputed = pd.DataFrame(imputed_array, index=pivot_df.index, columns=pivot_df.columns)

    # Continue with PCA steps ...

    
    # PCA based on Spearman correlation matrix
    spearman_corr = pivot_df_imputed.corr(method='spearman')
    pca_spearman = PCA(n_components=2)
    spearman_scores = pca_spearman.fit_transform(spearman_corr)
    
    
    csv_dateiname = f"{datumsteil}_00_df_PCA_spearman_corr.csv"
    csv_pfad = os.path.join(ordnername, csv_dateiname)
    spearman_corr.to_csv(csv_pfad, index=False)  # index=False to avoid saving
    

    import matplotlib.pyplot as plt
    import seaborn as sns
    
    # Calculate number of analytes (matrix size)
    num_analytes = spearman_corr.shape[0]
    
    # Set figure size dynamically based on number of analytes
    figsize = (max(12, num_analytes * 0.4), max(10, num_analytes * 0.4))
    ...
    sns.heatmap(
        ...
        annot_kws={"size": 6}  # Font size of numbers
    )
    
    plt.title("Spearman Correlation Between Analytes", fontsize=14)
    plt.xticks(rotation=90, fontsize=6)
    plt.yticks(rotation=0, fontsize=6)
    plt.tight_layout()
    
    # Save the figure
    plt.savefig("spearman_corr_visual.png", dpi=300)
    plt.close()
    ...
    # Save to file — PNG, high re

    
    # # PCA based on Z-score normalized analyte concentrations
    # scaler = StandardScaler()
    # z_scaled = scaler.fit_transform(pivot_df_imputed)
    # z_scaled_df = pd.DataFrame(z_scaled, index=pivot_df_imputed.index, columns=pivot_df_imputed.columns)
    # pca_zscore = PCA(n_components=2)
    # zscore_scores = pca_zscore.fit_transform(z_scaled_df.T)  # transpose to analyze analytes
    
    # # Plotting results
    # analyte_labels = pivot_df_imputed.columns
    # fig, axs = plt.subplots(1, 2, figsize=(18, 10))
    # offset = 0.01
    # fontsize = 6
    
    # # Spearman PCA plot
    # axs[0].scatter(spearman_scores[:, 0], spearman_scores[:, 1], c='skyblue')
    # for i, label in enumerate(analyte_labels):
    #     axs[0].text(spearman_scores[i, 0] + offset, spearman_scores[i, 1] + offset, label, fontsize=fontsize)
    # axs[0].set_title(f"PCA Based on Spearman Correlation ({time_tag} samples)")
    # axs[0].set_xlabel(f"PC1 ({pca_spearman.explained_variance_ratio_[0]:.1%} variance)")
    # axs[0].set_ylabel(f"PC2 ({pca_spearman.explained_variance_ratio_[1]:.1%} variance)")
    
    # # Z-score PCA plot
    # axs[1].scatter(zscore_scores[:, 0], zscore_scores[:, 1], c='lightcoral')
    # for i, label in enumerate(analyte_labels):
    #     axs[1].text(zscore_scores[i, 0] + offset, zscore_scores[i, 1] + offset, label, fontsize=fontsize)
    # axs[1].set_title(f"PCA Based on Z-score Normalization ({time_tag} samples)")
    # axs[1].set_xlabel(f"PC1 ({pca_zscore.explained_variance_ratio_[0]:.1%} variance)")
    # axs[1].set_ylabel(f"PC2 ({pca_zscore.explained_variance_ratio_[1]:.1%} variance)")
...
# Save
dateiname = f"{datumsteil}_PCA.png"
pfad = os.path.join(ordnername, dateiname)
plt.savefig(pfad, bbox_inches='tight')
plt.show()
plt.close()
