...
ordnername = f"{datumsteil}_Kalibrierung_SIM_Korrektur2_Versuch_logarithmisch"              ###########################################################################
...
#%%
# =============================================================================
# Korrekturfunktion aus den dC Proben plotten und errechnen. LOGARITMISCH
# =============================================================================
# =============================================================================
# Logarithmische Korrekturfunktion fitten und plotten
# =============================================================================
import os
import numpy as np
import pandas as pd
...
# Hier definieren wir x_shift = x - x_min + 1, sodass alle Werte > 0 sind.
def logarithmic(x, a, b):
    x_shift = x - np.min(x) + 1  # Verschiebung, damit das Argument immer > 0 ist
    return a * np.log(x_shift) + b

# Annahme: df, t0 etc. sind bereits definiert
for analyt_name, group in df.groupby(analyte_col):
    ...
    x_seconds = (x_time - t0).dt.total_seconds().values  # als numpy array

    # Fitten der logarithmischen Funktion
    params, _ = curve_fit(logarithmic, x_seconds, y_area, p0=[1, np.mean(y_area)])
    a, b = params

    # Berechnung der Fit-Kurve
    ...
    # Berechnung von R²
    residuals = y_area - logarithmic(x_seconds, a, b)
    ...
    plt.plot(x_fit_datetime, y_fit, color='red', label=f"y = {a:.3e}·log(x_shift) + {b:.2f}\nR² = {r2:.3f}")
    plt.title(f"{analyt_name} – Logarithmische Korrekturfunktion")
    ...
    # Achsen-Format etc. (wie im Original)
    # ...
    
    # Speichern und Anzeigen
    dateiname = f"{datumsteil}_{analyt_name}_SIM_Korrekturfunktion_logarithmisch.png"
    pfad = os.path.join(ordnername, dateiname)
    plt.savefig(pfad, bbox_inches='tight')
    plt.show()
    plt.close()

    # In DataFrame speichern (Formel als String)
    formel_str = f"y = {a:.3e}·log(x_shift) + {b:.2f}"
