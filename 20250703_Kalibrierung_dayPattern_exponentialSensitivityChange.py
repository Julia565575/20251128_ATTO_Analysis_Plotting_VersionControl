...
# For each analyte, plot a time series
for analyt_name, group in df.groupby(analyte_col):
    dC_Korrektur = group[group[datafile_col].str.contains('_dC_', na=False)]

    # Extract time and peak area from the already formatted column
    ...
    # Time relative to the chosen point (in seconds)
    ...
    try:
        popt, pcov = curve_fit(exponential_func, x_seconds, y_area, p0=(1, -1e-6, y_area.min()))
    except RuntimeError:
        continue
    ...
    plt.plot(x_fit_datetime, y_fit, color='red',
             label=f"y = {popt[0]:.5f}·exp({popt[1]:.5f}·t) + {popt[2]:.5f}")
    ...
    # Fit equation as string
    formel_str = f"y = {popt[0]:.5f}·exp({popt[1]:.5f}·t) + {popt[2]:.5f}"

    # Insert into DataFrame for all rows with this analyte
...
# Function to calculate the correction value per row
def berechne_korr_zeitwert(row):
    ...
    try:
        ...
        match = re.match(r"y\s*=\s*([-+]?[0-9]*\.?[0-9]+)·exp\(([-+]?[0-9]*\.?[0-9]+)·t\)\s*\+\s*([-+]?[0-9]*\.?[0-9]+)", fkt)
        if match:
...
# Initialize new column for expected concentration
df['expected_conc'] = np.nan

# For each analyte, get the exp_conc value from _dC_ rows
for analyt_name, group in df.groupby('analyte_col'):
...
# New column for corrected calibration function
...
for idx, row in df.iterrows():
    try:
        ...
        # Calculate slope: m = (y1 - y0) / (x1 - x0), where y0 = intercept at x0 = 0
        m = (y1 - intercept) / x1
        b = intercept
